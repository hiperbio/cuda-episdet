/**
 * CUDA application for high-order exhaustive epistasis detection using K2 scoring.
 *
...
void cuda_launch_kernel(int numSNPs, int controlsSize, int casesSize, int numCombinationsPerKernelInstance, int CpuThreadID, cudaStream_t stream_id)
{
	...
	epistasis<<<blocksPerGrid, WORKGROUP_SIZE, 2 * WORKGROUP_SIZE * comb * sizeof(ushort), 0>>>(d_datasetCases, d_datasetControls, d_combinationArray + (CpuThreadID * NUM_COMB_PER_KERNEL_INSTANCE * EPISTASIS_SIZE), d_lgammaPrecalc, d_output + CpuThreadID, d_output_index + CpuThreadID, EPISTASIS_SIZE, numSNPs, (int) ceil(casesSize / 64.0f), (int) ceil(controlsSize / 64.0f), numCombinationsPerKernelInstance, comb);
...
int main(int argc, char *argv[])
{
	...
	for(unsigned long long j = 0; j < (unsigned long long) ceil(numCombinations / (double) NUM_COMB_PER_KERNEL_INSTANCE); j++) {
		...
		int ret = cudaMemcpyAsync(d_combinationArray + (omp_thread_id * NUM_COMB_PER_KERNEL_INSTANCE * EPISTASIS_SIZE), combinationArray + (omp_thread_id * NUM_COMB_PER_KERNEL_INSTANCE * EPISTASIS_SIZE), numCombToGenerate * EPISTASIS_SIZE * sizeof(uint), cudaMemcpyHostToDevice, 0);

		outputFromGpu[omp_thread_id] = FLT_MAX;
		cudaMemcpyAsync(d_output + omp_thread_id, outputFromGpu + omp_thread_id, sizeof(float), cudaMemcpyHostToDevice, 0);

		cuda_launch_kernel(numSNPs, controlsSize, casesSize, numCombToGenerate, omp_thread_id, 0);

		cudaMemcpyAsync(outputFromGpu + omp_thread_id, d_output + omp_thread_id, sizeof(float), cudaMemcpyDeviceToHost, 0);
		cudaMemcpyAsync(output_indexFromGpu + omp_thread_id, d_output_index + omp_thread_id, sizeof(int), cudaMemcpyDeviceToHost, 0);
